 ...
 *
 * Factory names are all-caps and streaming methods mostly lowercase, so a node never
 * looks like a field: `h5::INPUT('email')->label('Email')` reads as a tag with
 * attributes, the same split HTML itself uses. Members that name a statement or a node
 * block follow the node spelling and are spelled in caps (`->THEN()`, `->ELSE()`,
 * `->BODY()`, `->AS()`, `->INDEX()`), because they name structure rather than
 * attributes. Note that PHP method names are case-insensitive, so the lowercase spelling
 * keeps working; `tests/FactoryNamingTest.php` is what actually holds the convention in
 * place.
 *
